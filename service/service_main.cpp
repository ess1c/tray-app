...
static bool ApplyTicketResponse(const std::string& body)
{
    ...
    /* Период обновления рассчитывается из ticketLifetime, но ограничен сверху,
       чтобы изменения статуса лицензии (блокировка/отзыв) подхватывались быстро. */
    const long long MAX_REFRESH_PERIOD = 30;  // сек
    if (ttl <= 0 || ttl > MAX_REFRESH_PERIOD) ttl = MAX_REFRESH_PERIOD;
    ...
    if (blocked) {
        ...
        g_state.licenseRefreshAt = Now() + 30;
        return false;
    }

    g_state.hasLicense       = true;
    g_state.licenseTicket    = body;
    g_state.expirationDate   = exp;
    g_state.licenseRefreshAt = Now() + ttl;
    return true;
}
